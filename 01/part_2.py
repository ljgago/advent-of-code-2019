...
def fuel_module(mass):
    fuel_remain = math.floor(mass / 3) - 2
    if fuel_remain > 0:
        return fuel_remain + fuel_module(fuel_remain)
    else:
        return 0

filename = open("d1input.txt", "r")
input_data_str = filename.read()
input_data = list(map(int, input_data_str.splitlines()))

print(fuel_module(1969))
print(fuel_module(100756))

fuel = 0
for mass in input_data:
    fuel += fuel_module(mass)
