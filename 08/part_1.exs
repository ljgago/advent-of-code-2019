...
defmodule PartOne do
  def image_check(layers) when is_list(layers) do
    layer_min_zero = do_min_digit_zero(layers)
    one_count = do_count_digit(layer_min_zero, "1")
    two_count = do_count_digit(layer_min_zero, "2")
    one_count * two_count
  end

  defp do_min_digit_zero(layers) when is_list(layers) do
    Enum.min_by(layers, fn layer ->
      do_count_digit(layer, "0")
    end)
  end

  defp do_count_digit(layer, digit) when is_binary(layer) and is_binary(digit) do
      layer
      |> String.graphemes
...
IO.puts("Number of 1 digits multiplied by the number of 2 digits of the layer whit the minor number of 0 digits:")
